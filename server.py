...
class DocBot:
    ...
    def generate_response(self, query: str, relevant_docs: List[Dict]) -> str:
        """Generate a response based on relevant documentation in bullet points"""
        ...
        for sentence in sentences:
            ...
            if any(term in sentence_lower for term in query_terms):
                # Limit sentence length to approximately 80 words
                words = sentence.split()
                if len(words) > 80:
                    sentence = ' '.join(words[:80]) + '...'
                relevant_sentences.append(sentence)
                if len(relevant_sentences) >= 5:  # Get more points
                    break
        
        # If we found relevant sentences, format as bullet points
        if relevant_sentences:
            for sentence in relevant_sentences:
                response += f"• {sentence.strip()}\n"
        else:
            # Fallback: use first meaningful sentences as bullet points
            meaningful_sentences = [s for s in sentences if len(s) > 30][:5]
            if meaningful_sentences:
                ...
                # Last resort: break content into chunks (max 80 words each)
                words = content.split()
                chunk_size = 80
                for i in range(0, min(len(words), 400), chunk_size):
                    ...
                    if i >= 320:  # Stop after ~4-5 points
                        break
